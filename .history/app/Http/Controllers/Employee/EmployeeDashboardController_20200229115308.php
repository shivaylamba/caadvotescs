   ...
   public function genQuotation(Request $request) 
    {
        ...
        $customer = DB::select('select * from leads where cust_id = ?', [$cust]);
        Mail::to($customer[0]->email)->send(new ServiceQuotation());
        return view('employee.quotation')->with(compact('ticket'))->with('status', 'Quotation Sent Successfully');;
   }
