...
class LoginController extends Controller
{
  ...
  public function __construct()
  {
    ...
    public function login(Request $request)
    {
      ...
      if (Auth::guard('employee')->attempt(['emp_email' => $request->emp_email, 'password' => $request->password, 'emp_type' => 'sales'], $request->remember)) {
        return redirect('/employee/dashboard');
        /*$email = $request->emp_email;
        $type = DB::select('select emp_type from employees Where emp_email = ?', [$email]);
        if( $type == 'sales') 
        {
            return redirect('/employee/dashboard');
        } 
      ...
      elseif (Auth::guard('employee')->attempt(['emp_email' => $request->emp_email, 'password' => $request->password, 'emp_type' => 'operation'], $request->remember)) {
        return redirect('/operation/dashboard');
      }
