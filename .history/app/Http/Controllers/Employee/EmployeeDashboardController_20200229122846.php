   ...
   public function genQuotation(Request $request) 
    {
        ...
        $ticket = DB::select('select * from leads where cust_id = ?',[$ticketno]);
        Mail::to($ticket[0]->email)->send(new ServiceQuotation());
        return redirect('/employee/sendQuote')->with(compact('ticket')->with('status', 'Quotation Sent Successfully');
   }
