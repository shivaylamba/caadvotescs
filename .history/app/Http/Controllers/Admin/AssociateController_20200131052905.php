...
class AssociateController extends Controller
{
    ...
    public function FeedbackList() 
    {
        ...
        return view('admin.feedbacks-list')->with(compact('feedbacks'));
    }
    ...
    public function FeedbackApprove(Request $request, $id) 
    {
        $status = 'approved';
        $updated_at = new \DateTime();
        DB::update('update feedbacks set status = ?,updated_at=? where id = ?',[$status,$updated_at,$id]);
        return redirect('/admin/feedbacks/list')->with('status', 'Feedback Approved Succesfully');
    }

    public function deleteFeedback(Request $request, $id) 
    {
        DB::delete('delete from feedbacks where id = ?',[$id]);
        return redirect('/admin/feedback/list')->with('status', 'Associate Account Deleted Succesfully');
    }
}
