...
class ServicesController extends Controller
{
    ...
    public function serviceedit(Request $request, $id) 
    {
        ...
        return view('admin.services.service-edit',['service'=>$service]);
    }
    ...
    public function updateservices(Request $request, $id) 
    {
        ...
        $status = $request->input('status');
        $updated_at = new \DateTime();
        DB::update('update services set servicename = ?, subcategory=?, content=?, status=?,updated_at=? where id = ?',[$servicename, $subcategory,$status,$updated_at,$id]);
        return redirect('/subcategory-list')->with('status', 'SubCategory Updated Succesfully');
    }
}
