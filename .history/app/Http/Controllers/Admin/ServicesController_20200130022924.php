...
class ServicesController extends Controller
{
    ...
    public function updateservices(Request $request, $id) 
    {
        ...
        $popular = $request->input('popular');
        $updated_at = new \DateTime();
        DB::update('update services set serviceimage = ?, servicename = ?, subcategory=?, content=?, status=?,updated_at=? , popular=? where id = ?',[$name, $servicename, $subcategory, $content, $status,$updated_at,$popular, $id]);
        return redirect('/services-list')->with('status', 'Service Updated Succesfully');
    }

    //Services Listed Settings

    public function servicesApprovals() 
    {
        $associate = DB::select('select * from associates');
        $approvals = DB::select('select * from hosted_services WHERE status != "approved"');
        return view('admin.services.services-approvals')->with(compact('approvals','associate'));
    }

    public function servicesApproved(Request $request, $id) 
    {
        $status = 'approved';
        $updated_at = new \DateTime();
        DB::update('update hosted_services set status = ?,updated_at=? where id = ?',[$status,$updated_at,$id]);
        return redirect('/admin/services/approvals')->with('status', 'Service Listning by Associate Approved Succesfully');
    }
    ...
    public function totalApproved() 
    {
        $associate = DB::select('select * from associates');
        $approvals = DB::select('select * from hosted_services WHERE status = "approved"');
        return view('admin.services.services-approved-lists')->with(compact('approvals','associate'));
    }
    ...
    public function createfaq() 
    {
        ...
        return view('admin.services.faq')->with(compact('services'));
    }
    ...
    public function savefaq(Request $request) 
    {
        ...
        $created_at = new \DateTime();
        $updated_at = new \DateTime();
        $data=array('serviceslug'=>$slug, 'servicename'=>$servicename, 'question'=>$question,'answer'=>$answer, 'created_at'=>$created_at, 'updated_at'=>$updated_at);
        DB::table('faqs')->insert($data);
        return redirect('/admin/faq/create')->with('status', 'FAQ Added Successfully');
    }

    public function listfaq() 
    {
        $faq = DB::select('select * from faqs');
        return view('admin.services.faq-list')->with(compact('faq'));
    }
}
