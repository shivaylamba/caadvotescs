...
class ServicesController extends Controller
{
    ...
    public function updatecategory(Request $request, $id) 
    {
        ...
        $status = $request->input('status');
        $updated_at = new \DateTime();
        DB::update('update servisecategories set categoryname = ?,lstatus=?,updated_at=? where id = ?',[$categoryname,$status,$updated_at]);
        return redirect('/category-edit')->with('status', 'Category Deleted Succesfully');
    }
