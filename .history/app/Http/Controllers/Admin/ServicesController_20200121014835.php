...
class ServicesController extends Controller
{
    ...
    public function saveservices(Request $request) 
    {
        $this->validate($request, [
            'input_img' => 'required|image|mimes:jpeg,png,jpg,bmp,gif,svg|max:2048',
          ]);
        ...
        $created_at = new \DateTime();
        $updated_at = new \DateTime();
        $data=array('servicename'=>$servicename,'status'=>$status, 'created_at'=>$created_at, 'updated_at'=>$updated_at, 'subcategory'=>$subcategory, 'content'=>$content, 'slug'=>$slug);
        DB::table('services')->insert($data);
        return redirect('/create-services')->with('status', 'Service Created Successfully')->with('status','You have successfully uploaded image.')
