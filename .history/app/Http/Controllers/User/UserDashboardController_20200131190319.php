...
class UserDashboardController extends Controller
{
   ...
   public function servicesList() 
    ...
    public function userLeads(Request $request) 
    {
        ...
        $created_at = new \DateTime();
        $updated_at = new \DateTime();
        $data=array('category'=>$category, 'service'=>$service, 'state'=>$state,'city'=>$city, 'name'=>$name, 'mobile'=>$mobile,'email'=>$email, 'created_at'=>$created_at, 'updated_at'=>$updated_at);
        DB::table('leads')->insert($data);
        return redirect('/user/respond')->with('status', 'Thank your for submiting your feedback');
    }
}
