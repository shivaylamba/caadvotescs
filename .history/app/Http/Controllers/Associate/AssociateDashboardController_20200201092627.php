...
class AssociateDashboardController extends Controller
{
   ...
   public function govfeesList() 
    ...
    public function associateSkillsSubmit(Request $request) 
    {
        ...
        $created_at = new \DateTime();
        $updated_at = new \DateTime();
        $data=array('associate_email'=>$email,'skills'=>'skills', 'created_at'=>$created_at, 'updated_at'=>$updated_at);
        DB::table('associate_skills')->insert($data);
        return redirect('/associate/skills')->with('status', 'Skill added Successfully');
