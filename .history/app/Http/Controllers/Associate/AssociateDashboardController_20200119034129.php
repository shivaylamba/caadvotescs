...
class AssociateDashboardController extends Controller
{
    ...
    public function submitService(Request $request)
    {
        ...
        $data=array('requirements'=>$requirements,'status'=>'unapproved', 'created_at'=>$created_at, 'updated_at'=>$updated_at, 'deliverables'=>$deliverables, 'timeline'=>$timeline, 'price'=>$price, 'byAssociate'=>$byAssociate);
        DB::table('hosted_services')->insert($data);
        return redirect('/associate/service/host')->with('status', 'Service Submitted to the Admin Successfully for the Approvel');
    }
}
