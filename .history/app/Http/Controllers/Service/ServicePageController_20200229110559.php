...
class ServicePageController extends Controller
{
    ...
    public function userLeads(Request $request) 
    {
        ...
        $cust_id = ('CUST'.str_shuffle($pin)); 
        $created_at = new \DateTime();
        $updated_at = new \DateTime();
        
        if( DB::select('select email from users WHERE email = ?',[$email])){

            $data=array('cust_id'=>$cust_id, 'category'=>$category, 'service'=>$service, 'state'=>$state,'city'=>$city, 'name'=>$name, 'phone'=>$phone,'email'=>$email, 'created_at'=>$created_at, 'updated_at'=>$updated_at);
            ...
            $footerps = DB::select('select * from services where status = "active" and popular="yes" LIMIT 6');
            $footercategory = DB::select('select * from servisecategories WHERE status = "active" LIMIT 6');
            Mail::to($email)->send(new QuotationLogin());
        }
